...
TFEnrichment = function(TF.name,cluster.counts,TF.df){
  ...
  for(i in 1:length(clusters))
    {X = matrix(c(cluster.TF.counts[i],sum(cluster.TF.counts[-i]),cluster.counts[i]-cluster.TF.counts[i],sum(cluster.counts[-i])-sum(cluster.TF.counts[-i])),ncol=2,byrow=T)
    X[is.na(X)] <- 0
    pvals[i] = fisher.test(X,alternative='greater')$p.value
    }
  return(pvals)}
